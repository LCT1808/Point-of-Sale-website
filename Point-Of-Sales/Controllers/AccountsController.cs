    ...
    public class AccountsController : Controller
    {
        ...
        public async Task<IActionResult> Create(AccountModel account)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result > 0)
                {
                    string subject = "[1 Minutes] - Activate your staff account!!!";
                    var link = $"{Request.Scheme}://{Request.Host}" + Helpers.HelperConfirm.Generatelink(username);

                    string content = $"Welcome to our store's <b>MULTI-CATEGORY RETAIL STORE MANAGEMENT INFORMATION SYSTEM</b> website.<hr>Please click <a href=\"{link}\"> here</a> to active your account.<br><br>What a collaboration!<br><br>T&T Chain Store";
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            var account = await _context.Accounts.FindAsync(id);
            var employee = _context.Employees.FirstOrDefault(e => e.AccountId.Equals(id));
        ...
        {
            var customer = _context.Customers.FirstOrDefault(c => c.Email.Equals(q));
            if (customer == null) return Ok();
            return Ok(new { email = customer.Email});
        }
